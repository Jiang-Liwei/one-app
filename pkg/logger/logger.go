...
// InitLogger 日志初始化
func (logger *LoggerStruct) InitLogger() {

	// 获取日志写入介质
	writeSyncer := logger.getLogWriter()
	// 设置日志等级
	logLevel := new(zapcore.Level)
	if err := logLevel.UnmarshalText([]byte(logger.Level)); err != nil {
...
func (logger *LoggerStruct) getLogWriter() zapcore.WriteSyncer {

	// 如果配置了按照日期记录日志文件
	if logger.LogType == "daily" {
		logname := time.Now().Format("2006-01-02.log")
		logger.Filename = strings.ReplaceAll(logger.Filename, "logs.log", logname)
	}
